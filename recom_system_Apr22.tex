...
\begin{abstract}
We study a platform that shapes user preferences over time through its recommendation decisions. Using the Microsoft News Dataset (MIND), we document four empirical regularities: (i)~an inner-product utility model predicts clicks with AUC 0.707; (ii)~user taste shifts toward consumed content, with cosine alignment $+0.36$ ($z = 515$); (iii)~click probability decays sharply with taste-content distance, implying a local persuasion radius; and (iv)~taste change is driven by consumption, not mere exposure; shown-but-not-clicked content pushes taste \emph{away} (Cohen's $d = -0.75$).

Motivated by these findings, we formulate a dynamic optimization model in which a platform recommends content to a user whose taste vector evolves on the probability simplex. Clicked content pulls taste toward the recommended item; ignored content may push taste away. With heterogeneous category-level revenues, the platform faces a three-way tradeoff between exploitation (maximizing immediate clicks), exploration (learning preferences), and cultivation (steering taste toward high-value categories through locally persuasive steps). We characterize the structure of the optimal policy and show that the cultivation motive can dominate both exploitation and exploration, leading to recommendation paths that differ qualitatively from myopic or bandit-style policies.
\end{abstract}
...
Modern recommendation systems do more than match users to content they already prefer. By choosing what to display, a platform influences what users consume, and consumption reshapes what users will want next. This feedback loop means that today's recommendation is also an investment in tomorrow's demand.
...
\item \textbf{Does current taste predict clicks?} Yes. A multinomial logit model using only the cosine similarity between a user's historical taste vector and an article's feature vector achieves an out-of-sample AUC of 0.707. This validates the inner-product utility representation that our model adopts.

\item \textbf{Does consumption change future taste?} Yes. In a three-period design (baseline, intermediate, outcome), user taste in the outcome period shifts strongly toward content consumed in the intermediate period, with a mean cosine alignment of $+0.360$ ($z$-score $= 515$). A placebo test using earlier consumption shows near-zero alignment ($-0.008$); although statistically significant due to the large sample ($t = -8.1$), the magnitude is economically negligible. In a shorter 7-day dataset, the main statistic is insignificant, indicating that taste change is gradual and requires time to accumulate.

\item \textbf{Is persuasion local?} Yes. Click-through rates decay 3.4-fold from the nearest to the farthest content in taste space. This implies a persuasion radius: content too far from current taste cannot generate clicks and therefore cannot move taste.

\item \textbf{What is the mechanism?} Consumption, not exposure. Clicked content predicts subsequent taste growth in the relevant category ($\Delta\theta = +0.056$, $t = 301$). Shown-but-not-clicked content predicts taste \emph{decline} ($\Delta\theta = -0.024$, $t = -366$), a backfire effect. The within-user difference between clicked and not-clicked effects has Cohen's $d = -0.75$, confirming that engagement, not mere exposure, is the operative mechanism.
\end{enumerate}

These findings motivate a model in which: (a)~taste is a probability distribution over content categories; (b)~the platform recommends one item per period; (c)~the user clicks with a probability determined by taste-content alignment; (d)~clicked content pulls taste toward the item, while ignored content may push taste away; and (e)~heterogeneous category-level revenues create a motive to cultivate taste toward high-value categories.
...
We use the MINDlarge release of the Microsoft News Dataset, which covers November 9--22, 2019 (14 days), with approximately one million users, 2.6 million impressions in the training set, and 2.4 million in the test set. Articles belong to 15 broad categories (news, sports, lifestyle, etc.) and 285 subcategories.
...
The dynamic analyses use a three-period design: Period~1 (P1, Nov~9--11) measures baseline taste; Period~2 (P2, Nov~12--15) is the intermediate period in which the platform recommends and users consume; Period~3 (P3, Nov~16--22) measures subsequent taste.
...
The placebo is statistically significant ($t = -8.1$) but economically negligible in magnitude ($-0.008$ vs.\ $+0.360$), likely reflecting weak mean-reversion in taste. A permutation test (1,000 random shuffles) yields a $z$-score of 515 for the main alignment. In the shorter 7-day MINDsmall dataset, the same statistic is $-0.012$ and insignificant, confirming that taste evolution is gradual. The alignment result is robust to the choice of metric: replacing cosine with raw inner product, Spearman rank correlation, or component-wise sign agreement yields qualitatively identical conclusions (Table~\ref{tab:metric_robust}). Using distribution-native distances (Hellinger, total variation, Jensen-Shannon), 98.6\% of users move closer to clicked content from P1 to P3 (Table~\ref{tab:dist_robust}).
...
\subsection{Finding 3: Local persuasion radius}

Click probability decays sharply with cosine distance between user taste and article features. CTR drops from 9.75\% for the nearest content (mean distance 0.294) to 2.86\% for the farthest (distance 1.000), a 3.4-fold decline. This implies a practical persuasion radius: the platform can only generate clicks, and therefore only move taste, through content sufficiently close to the user's current interests. Moving taste across large distances requires a sequence of intermediate steps.

\subsection{Finding 4: Consumption, not exposure, drives taste change}

The critical mechanism question is whether recommendation shapes taste through exposure (showing content) or through engagement (the user clicking and consuming it). We decompose content into three groups: clicked, shown-but-not-clicked, and never-shown. For each user-category pair, we measure the change in category-level taste share from P1 to P3. The results (145,985 users) are:
...
Neither (control) & $-0.006$ & $-101$ & 353,773 \\
\bottomrule
\end{tabular}
\end{center}

Clicked content increases subsequent taste share, confirming consumption-driven taste change. Shown-but-not-clicked content \emph{decreases} taste share relative to the never-shown control, with a within-user paired $t = -287$ and Cohen's $d = -0.75$. This backfire effect is consistent with advertising fatigue or Berlyne's two-factor theory: repeated exposure without engagement may generate satiation or active avoidance. The backfire direction is confirmed under all four alternative alignment metrics tested in the robustness analysis (Table~\ref{tab:metric_robust}).

A decomposed OLS regression reinforces these findings:
\[
\theta_{i,k}^{(\text{P3})} = \beta_0 + \beta_1 \theta_{i,k}^{(\text{P1})} + \beta_2 \, \text{clicked\_frac}_{i,k} + \beta_3 \, \text{shown\_nc\_frac}_{i,k} + \varepsilon_{i,k}.
\]
The estimates ($N = 1{,}852{,}089$ user-category pairs, $R^2 = 0.94$) are $\hat{\beta}_1 = 0.41$ ($t = 804$), $\hat{\beta}_2 = 0.51$ ($t = 998$), and $\hat{\beta}_3 = 0.07$ ($t = 104$). The clicked-content coefficient is 7.6 times larger than the shown-not-clicked coefficient, confirming that consumption is the dominant driver.
...
\item Click probability is well-described by an inner-product (cosine) utility model.
\item Taste updates are consumption-driven, not exposure-driven: the platform's influence operates through the click channel.
\item Persuasion is local: moving taste requires a sequence of small steps through intermediate content.
\item There is a backfire risk: showing content that the user does not click can push taste away from that category.
\end{itemize}
...
The simplex restriction reflects the empirical finding that taste is redistributed across categories rather than expanded: when a user's interest in one category grows, interest in other categories declines proportionally.
...
The user's taste evolves depending on whether the recommended content is clicked. A single deterministic transition rule (e.g., $\boldsymbol{\theta}_{t+1} = \lambda \boldsymbol{\theta}_t + (1-\lambda) \mathbf{a}_t$) cannot accommodate the empirical finding that clicked and non-clicked recommendations have opposite effects on taste. The data show that clicked content pulls taste toward the recommended category, while shown-but-not-clicked content pushes taste away. We therefore specify a two-branch transition that conditions on the click outcome.
...
If the user does \emph{not click}, taste updates as
\begin{equation}\label{eq:noclick_transition}
\boldsymbol{\theta}_{t+1}^{\text{no click}} = \frac{1}{Z}\bigl(\boldsymbol{\theta}_t + \mu(\boldsymbol{\theta}_t - \mathbf{a}_t)\bigr)^{+},
\end{equation}
...
The click transition~\eqref{eq:click_transition} is a convex combination that pulls taste toward the consumed item, consistent with the empirical finding that clicked content shifts taste in its direction. The persistence parameter $\lambda$ controls the speed of taste change.

The no-click transition~\eqref{eq:noclick_transition} captures the backfire effect: when content is shown but not consumed, taste moves \emph{away} from the recommended category. The parameter $\mu = 0$ recovers the case of no backfire (taste is unchanged after a non-click). When $\mu > 0$, ignored recommendations actively push taste away, consistent with the empirical finding that shown-not-clicked categories experience taste decline. The projection $(\cdot)^{+}$ and renormalization are needed because for large $\mu$, the unconstrained vector $\boldsymbol{\theta}_t + \mu(\boldsymbol{\theta}_t - \mathbf{a}_t)$ may have negative components; the projection ensures the result lies on the simplex after normalization.
...
The decomposed OLS regression (Section~\ref{sec:empirical}) yields $\hat{\beta}_1 = 0.41$ for the baseline-taste coefficient and $\hat{\beta}_2 = 0.51$ for the clicked-content coefficient. Under the linear click transition~\eqref{eq:click_transition}, the natural mapping is $\lambda \approx \hat{\beta}_1 / (\hat{\beta}_1 + \hat{\beta}_2) \approx 0.45$. The fact that $\hat{\beta}_1 + \hat{\beta}_2 \ne 1$ (they sum to $0.92$, not $1.0$) reflects the presence of other covariates in the regression, including the shown-not-clicked fraction and the intercept. The mapping is therefore approximate, and the numerical study will explore sensitivity to $\lambda$.
\end{remark}
...
\end{equation}
The expected taste change is proportional to the click probability times the direction from current taste toward the recommended item. This highlights the role of the persuasion radius: distant items have low click probability and therefore contribute little to expected taste change, even though a successful click would produce a large taste update.
...
\paragraph{Consumption as the mechanism.} Our model makes taste change contingent on clicking, not on exposure. This is a direct consequence of Finding~4: shown-not-clicked content does not help (and may hurt) the platform's taste-cultivation objective. This modeling choice addresses a limitation of earlier formulations in which recommendation itself (i.e., exposure) was assumed to cause taste change. The empirical evidence is unambiguous: the clicked-content coefficient in the decomposed regression is 7.6 times larger than the shown-not-clicked coefficient, and the directional analysis shows that shown-not-clicked content pushes taste \emph{away} from the exposed category. A model that treated exposure as the mechanism would therefore mischaracterize the platform's influence and overstate its ability to steer taste.

\paragraph{Backfire.} The parameter $\mu$ captures the empirical backfire effect. In the base analysis, we first solve the model with $\mu = 0$ (no backfire) to isolate the cultivation mechanism, then introduce $\mu > 0$ to study how backfire risk changes the optimal policy. The key qualitative effect is to increase the cost of recommending content that the user is unlikely to click, making the platform more conservative in its cultivation strategy.
...
\item \textbf{Backfire intensity $\mu$.} The shown-not-clicked coefficient $\hat{\beta}_3 = 0.07$ and the Cohen's $d = -0.75$ for the backfire effect provide calibration targets for $\mu$.
...
\subsubsection{Shown but not clicked: the backfire}
...
\subsubsection{Position and the persuasion radius}
...
Three findings emerge. First, the category-level MNL AUC (0.696) retains 98.5\% of the subcategory-level performance (0.707). The loss from aggregating 285 subcategories into $K$ categories is small, confirming that the category-level model captures the dominant source of click-prediction variation.
...
The results are qualitatively identical across all four metrics. Clicked content aligns strongly with subsequent taste shift (cosine $+0.375$, Spearman $+0.486$, sign agreement 79.6\%). Shown-not-clicked content shows near-zero or negative alignment (cosine $-0.015$, inner product $-0.001$), confirming the backfire effect. The sign agreement metric provides a particularly intuitive check: for clicked content, 79.6\% of category-level taste changes point in the same direction as consumption, far exceeding the 50\% null. For shown-not-clicked content, sign agreement is 53.7\%, barely above chance, and the negative cosine and inner product values indicate that the dominant direction is away from the exposed content.
...
The backfire direction is further confirmed at the individual (user, category) level. Among 735,756 pairs where articles were shown but not clicked in a given category, 45.1\% of cases show taste moving away from that category, while 54.9\% show taste moving toward it. However, the magnitude of the away movement is significantly larger: mean $|\Delta\theta_k|$ is 0.144 for away cases versus 0.117 for toward cases ($t = 105.2$, $p < 10^{-300}$). Combined with the negative cosine alignment at the aggregate level, this confirms that shown-not-clicked exposure produces a net backfire effect.

\subsubsection{Distribution-native distance metrics}
\label{sec:dist_robustness}

A complementary robustness check uses distribution-native distance metrics to test whether taste moves \emph{closer} to consumed content over time. For each user, we compute $\Delta d = d(\boldsymbol{\theta}^{(\text{P3})}, \mathbf{c}) - d(\boldsymbol{\theta}^{(\text{P1})}, \mathbf{c})$, where $\mathbf{c}$ is a reference consumption/exposure vector and $d$ is one of four distance metrics: Hellinger distance, total variation ($\ell_1/2$), Jensen-Shannon divergence, or cosine distance ($1 - \cos$). Negative $\Delta d$ means taste moved closer to the reference from P1 to P3. This formulation avoids the issue that the taste \emph{shift} vector $\Delta\boldsymbol{\theta}$ is not a probability distribution and therefore not a natural argument for Hellinger or TV.
